...
$username_error = $password_error = $confirm_password_err = "";
 
// Processando dados do formulário quando o formulário é enviado
if($_SERVER["REQUEST_METHOD"] == "POST"){
 
    // Validar nome de usuário
    if(empty(trim($_POST["username"]))){ //empty determoma se uma variavel é considerada vazia
        $username_error = "Por favor coloque um nome de usuário.";
    } elseif(!preg_match('/^[a-zA-Z0-9_]+$/', trim($_POST["username"]))){
        $username_error = "O nome de usuário pode conter apenas letras, números e sublinhados.";
    } else{
        ...
        if($stmt = $pdo->prepare($sql)){
            ...
            if($stmt->execute()){
                if($stmt->rowCount() == 1){
                    $username_error = "Este nome de usuário já está em uso.";
                } else{
